...
def _zone_rotation_score(draws: List[List[int]], window: int = 8) -> Dict[int, float]:
    ...
    for n in ALL_NUMBERS:
        zone = min(4, (n - 1) // 10)
        deficit = expected - zone_counts[zone]
        if deficit > expected * 0.3:
            zone_scores[n] = deficit * 0.8
        else:
            zone_scores[n] = 0.0
    return zone_scores
...
def _pick_top_six_optimized(
    ...
    sum_tolerance: float = 30.0
) -> List[Tuple[int, int, float, str]]:
    """优化版6码筛选：整合区间轮动加分、和值趋势约束"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="香港六合彩预测工具 - 优化版（整合区间轮动/和值约束/特别号终极增强）")
