...
def _get_two_zodiac_from_history_rows(rows: Sequence[sqlite3.Row]) -> List[str]:
    ...
    if len(rows) > 1:
        ...
        if len(rows) > 2:
            ...
            prev_prev_special = int(prev_prev_draw["special_number"])
            prev_prev_zodiacs = [get_zodiac_by_number(n) for n in prev_prev_main]
            ...
            if len(rows) > 3:
                ...
                for z, _ in ranked_prev_prev:
                    if len(picks_prev_prev) >= 2: break
                    if z not in picks_prev_prev: picks_prev_prev.append(z)
