...
ML_MODEL_KEY = "lightgbm_model_newmacau"
...
def parse_macau_from_api(payload: dict) -> List[DrawRecord]:
    records = []
    ...
    if not macau:
        return records
    history = macau.get("history", [])
    for line in history:
        m = re.match(r"(\d{7})\s*期[：:]\s*([\d,]+)", line)
        if not m: continue
        expect_raw = m.group(1)
        ...
        issue = f"{year}/{seq.zfill(3)}"
        draw_date = _parse_date(macau.get("openTime", "").split()[0]) if macau.get("openTime") else "2026-01-01"
        records.append(DrawRecord(issue, draw_date, main, sp))
    return records


def fetch_macau_records(retries: int = 4, timeout: int = 20) -> List[DrawRecord]:
    req = Request(API_URL, headers={"User-Agent": "Mozilla/5.0", "Accept": "application/json"})
    for attempt in range(1, retries + 1):
        try:
            with urlopen(req, timeout=timeout) as resp:
                raw = resp.read().decode("utf-8-sig")
            records = parse_macau_from_api(json.loads(raw))
            if records:
                print(f"[sync] 获取到 {len(records)} 条澳门记录")
                return records
...
def _momentum_map(draws):
    ...
    for i, d in enumerate(draws):
        ...
        for n in d: m[n] += w
    return m
def _pair_affinity_map(draws, w=3):
    cnt = {}
    for d in draw[:w]:
        s = sorted(d)
...
def _ensemble_strategy_v3_1(draws, mined_config, strategy_weights, conn, issue_no):
    ...
    for sub in subs:
        ...
        sub_draws = draw[:win_size] if len(draws)>win_size else draw
...
def get_dynamic_final_recommendation(conn):
    ...
    trio = get_trio_from_merged_pool20(conn, next_iss)
    conf = min(98, max(60, int( sum(rows[0]["avg_rate"] for rows in rows if rows) / len(rows) * 135 )))
    zodiac_two = get_two_zodiac_picks(conn, next_iss)
    zodiac_one = get_single_zodiac_pick(conn, next_iss)
...
def review_latest(conn):
    row = conn.execute("SELECT issue_no FROM draws ORDER BY draw_date DESC, issue_no DESC LIMIT 1").fetchone()
    if row:
        # 简化复盘：将 PENDING 更新为 REVIEWED 并计算命中
...
def cmd_bootstrap(args):
    ...
    sync_from_records(conn, records, "macau_api")
